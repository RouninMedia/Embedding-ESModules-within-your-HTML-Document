...
Let's take a closer look at the `async / await` code above to get a better idea about what's happening in each section:
 ...
 - **Single Export:** The _second_ easiest way to deploy the `parseEmbeddedModule()` function is to introduce, as the function's second argument, a `string` which indicates the name of _a single named export_ from the **embedded module**. In this case (as above) _all named exports_ from the activated module will become available from the global `embeddedModules` object but _only that specific named export_ will be available from the variable to which it has been assigned. Note that this behaviour might be desirable in only a limited set of situations.
