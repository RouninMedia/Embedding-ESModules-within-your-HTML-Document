...
Let's take a closer look at the `async / await` code above to get a better idea of what's happening in each section:
 ...
 - **Synchronous Retrieval:** Once named exports are synchronously available, it isn't necessary to run the `parseEmbeddedModule()` function again. In the code above we can see that `test_D1.displayParagraph_1(\`test_D1: ${test_D1.paragraph_1}\`);` works, even in the absence of the `parseEmbeddedModule()` function being invoked. Why? Because
