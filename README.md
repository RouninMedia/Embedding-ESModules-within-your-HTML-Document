...
Let's take a closer look at the `async / await` code above to get a better idea of what's happening in each section:
 ...
 - **This will not work:** We can try the same thing with ``test_B1.displayParagraph_1(`test_B1: ${test_B1.paragraph_1}`);`` but it won't work. Why not? --- In this case we would need to run -- or -- before
